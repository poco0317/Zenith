	...
	for indy=1,iTilesY do
		t[#t+1] = Def.Quad {
			...
			OnCommand=function(self)
				self:diffuse(Color("Black")):diffusealpha(1):zoom(fTileSize-2):sleep((iTilesX+iTilesY)*(1/60)):linear(0.0325 + ( indx / 60 )):diffusealpha(0):zoom(fTileSize*1.25)
			end;
		};
	end
end --]]
t[#t+1] = Def.Quad {
	...
	OnCommand=function(self)
		self:diffuse(color("0,0,0,1")):sleep(0.0325 + ( ( iTilesX/2 ) / 60 )):linear(0.15):diffusealpha(0)
	end;
};
